...
const Produto = require('../entities/produtos');
...
const getProdutoDB = async () => {
    try { 
        ...
        console.error("Error in getProdutoDB:", err); // Add this line for logging
        throw "Erro: " + err;
     }
}
...
const updateProdutoDB = async (body) => {
    try {
        ...
        const results = await pool.query(`
            UPDATE produtos 
            SET nome = $2, descricao = $3, quantidade_estoque = $4, ativo = $5, valor = $6, data_cadastro = $7, categoria = $8
            WHERE codigo = $1
            RETURNING codigo, nome, descricao, quantidade_estoque, ativo, valor, data_cadastro, categoria`,
            [codigo, nome, descricao, quantidade_estoque, ativo, valor, data_cadastro, categoria]
        );
        if (results.rowCount === 0) {
            throw `Nenhum registro encontrado com o código ${codigo} para ser atualizado`;
        }
        const produto = results.rows[0];
        return new Produto(
            produto.codigo,
        ...
        throw "Erro ao atualizar o produto: " + err;
    }
}
...
const getProdutoPorIdDB = async(codigo) =>{
    try{
        ...
        throw "Erro ao buscar o produto: " + err;
    }
}
